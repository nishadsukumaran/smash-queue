...
import { requestMagicLink, redeemMagicLink, signOut, pruneAuth } from "@/lib/auth";
...
export async function signInAction(_prev: SignInState, fd: FormData): Promise<SignInState> {
  ...
  const sent: SignInState = {
    status: "sent",
    message: "If that address is on the staff list, a sign-in link is on its way. It expires in 15 minutes.",
  };
  ...
  try {
    const res = await sendMagicLink(req.user.email, url, req.user.name);
    ...
    if (!res.delivered && process.env.NODE_ENV !== "production") {
      return { status: "sent", message: sent.message, devLink: res.fallbackLink };
    }
  } catch {
    return {
      status: "error",
